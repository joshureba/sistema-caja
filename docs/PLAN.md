...
## Regla vigente del 14/09 (reemplaza los cortes anteriores)

- Fuente: `CAJA AUTOMATIZADA 2026 (TEMPORAL SOLO POR HOY 14-09).xlsm`, 125 operaciones del 01 al 13/09. Se conservan cinco ingresos con monto cero como en el archivo y se anotan tres correcciones de año 2025 a 2026.
- Caja chica abre el 14/09 con el contado del cierre 13/09: S/ 6,032.90. Entrega por última vez S/ 500 para abrir la caja de fondo, quedando S/ 5,532.90 antes de nuevas operaciones.
- La caja de fondo (diaria) abre con S/ 500 solo el día del corte. Después la apertura es automática: saldo anterior + cobros en efectivo − envíos a gerencia. Se permite RETIRO de DIARIA con destino GERENCIA, registrado sin afectar caja chica. No permite financiar reposiciones. Los cobros digitales se contabilizan por separado. El conteo verifica el saldo; no lo reemplaza ni genera ingresos.
- Regla del 17/09: la caja chica admite egresos y reposiciones desde el banco; desde el 16/09 los retiros de efectivo solo salen de la caja de fondo (lo que sale de caja chica es egreso). Las restricciones se validan en formulario y base de datos.
- Los movimientos anteriores al corte siguen disponibles para consultas, pero no vuelven a afectar los saldos iniciales. El traslado antiguo del 11/09 queda en el histórico.
- El dashboard muestra solo operaciones activas desde el corte, hasta hoy y dentro del período seleccionado. Ingresos totales = cobros digitales + efectivo; excluye la base de apertura y no descuenta gastos de caja chica. Tarjetas: ingresos totales, ingresos digitales (Niubiz), ingresos en efectivo, egresos caja chica y movimientos. El histórico conserva todos los períodos anteriores.
- Se conservan los umbrales configurables de S/ 3,000 a S/ 5,000. El nuevo saldo supera el máximo; esa alerta no genera retiros automáticos.
- Migración aplicada: `20260914000000_actualizar_excel_y_separar_cajas.sql`. Respaldo privado previo: `respaldo_caja.antes_20260914`.
- Migración adicional: `20260914010000_fondo_arrastre_y_gerencia.sql`. Apertura calculada por trigger, base inicial de solo lectura y destino GERENCIA. La prueba transaccional `scripts/probar-fondo.sql` verifica el arrastre y revierte sus datos de prueba.

El resto de este documento conserva el diseño inicial del 11/09 como referencia histórica. En caso de diferencia rigen las reglas anteriores.
